...
public class ThreadLocalTest {
    private static void print(String str) {        //1.1  打印当前线程本地内存中localVariable变量的值
    ...
    public static void main(String[] args) {        //(3) 创建线程one
        Thread threadOne = new Thread(() -> {                //3.1 设置线程one中本地变量localVariable的值
            localVariable.set("threadOne local variable");                //3.2 调用打印函数
            print("threadOne");                //3.3打印本地变量值
            System.out.println("threadOne remove after" + ":" + localVariable.get());

        });        //(4) 创建线程two
        Thread threadTwo = new Thread(() -> {                //4.1 设置线程one中本地变量localVariable的值
